...
export default function HeroSection() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data?.value) setHero({ ...defaultHero, ...(data.value as any) });
    })();
  }, []);
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 mb-8 animate-fade-in-up">
          ...
          <span className="text-sm font-medium text-primary tracking-wide">{hero.badge}</span>
        </div>
          ...
          {words.map((word, i) => (
            <span
              ...
              className={`inline-block mr-3 md:mr-5 transition-all duration-700 ${
                i < visibleWords ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              } ${i === words.length - 1 ? "text-primary italic" : "text-foreground"}`}
            >
              {word}
            </span>
          ))}
